...
//        System.out.println("ServerPluginMessagePacketBehaviour::process => Received a plugin message packet for client <"+clientInstance.getUsername()+"> with channel <"+pluginMessagePacket.getChannel()+">");
...
//        PacketWrapper.setProcessed(packet, true);
		...
		if (pluginMessagePacket.getChannel().equals("Koekepan|migrate")) {
            ...
            System.out.println("ServerPluginMessagePacketBehaviour::process => Received a migration message for client <"+ clientInstance.getUsername()+"> to migrate to server <"+hostname+">");
//			Logger.log(this, Logger.Level.DEBUG, new String[]{"network", "behaviour", "migration"} ,"Received a migration message for client <"+ clientInstance.getUsername()+"> to migrate to server <"+hostname+">");
            clientInstance.migrate(hostname, 25565); // Port just stays the same as default
        }
